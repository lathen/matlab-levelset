...
void compute(mxArray * plhs[], const mxArray * prhs[])
{
    ...
    DataType * Dzm = static_cast<DataType *>(mxGetData(plhs[4]));
    DataType * Dzp = static_cast<DataType *>(mxGetData(plhs[5]));

    IndexType ind = 0;
    while (iter != iend) {

        Dxp[ind] = iter.get(0,1,0) - (*iter);
        Dxm[ind] = (*iter) - iter.get(0,-1,0);

        Dyp[ind] = iter.get(1,0,0) - (*iter);
        Dym[ind] = (*iter) - iter.get(-1,0,0);

        Dzp[ind] = iter.get(0,0,1) - (*iter);
        Dzm[ind] = (*iter) - iter.get(0,0,-1);
